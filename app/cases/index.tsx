...
export default function DiagnosticRoom() {
  const { subCategory, difficulty, score, deductPoints } = useStore();
      ...
      <Modal visible={!!activeMenu} transparent={true} animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.menuContainer}>
            ...
            <FlatList data={activeMenu ? medicalMenus[activeMenu] : []} keyExtractor={(item) => item} renderItem={({ item }) => (
                <Pressable style={styles.menuItem} onPress={() => handleSendRequest(item)}><Text style={styles.menuItemText}>{item}</Text></Pressable>
              )} />
            <Pressable style={styles.closeMenuBtn} onPress={() => setActiveMenu(null)}><Text style={styles.closeMenuText}>Cancel</Text></Pressable>
          </View>
        </View>
      </Modal>
